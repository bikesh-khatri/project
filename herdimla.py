    ...
    def register():
        ...
        try:
            ...
            for i in range(number_of_rooms):
                cursor.execute("INSERT INTO room (user_id, floor, capacity) VALUES (?, ?, ?)", (user_id, i + 1, 4))
...
def add_students(user_id):
    ...
    def fetch_rooms():
        conn = sqlite3.connect("new.db")
        cursor = conn.cursor()
        cursor.execute("SELECT id, floor, capacity, occupied FROM room WHERE user_id = ? AND occupied < capacity", (user_id,))
