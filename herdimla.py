...
def students(root):
    ...
    def edit_student():
        ...
        for i, label in enumerate(labels):
            ...
            if label == "ID":
                # Make the Student ID field read-only (non-editable)
        ...
        def save_edit():
            ...
            edit_win.destroy()
            load_students()  # Reload the table to reflect changes
...
def room():
    ...
    # Create a Treeview widget
    columns = ("Room no", "Floor", "Capacity", "Occupied", "Beds Left", "Students")
    tree = ttk.Treeview(table_frame, columns=columns, show="headings", selectmode="browse")
    tree.pack(fill="both", expand=True)
    ...
    def load_rooms():
        ...
        # Fetch room data
        cursor.execute("SELECT no, floor, capacity, occupied FROM room")
        rooms = cursor.fetchall()

        for room_data in rooms:
            room_no, floor, capacity, occupied = room_data
            ...
            tree.insert("", "end", values=(room_no, floor, capacity, occupied, beds_left, student_names))
    ...
    def edit_room():
        ...
        labels = ["Room no", "Floor", "Capacity", "Occupied"]
        entries = {}
        ...
        def save_edit():
            updated_data = [entries[label].get() for label in labels]
            room_no = updated_data[0]  # Room no is not editable, used for WHERE clause
            ...
            cursor.execute("""
                ...
                WHERE no = ?
            """, (updated_data[1], capacity, occupied, room_no))
            conn.commit()
            conn.close()
